...
//Fast Decouple Load Flow Analysis
...
string array[8][5];
float array_int[8][5];
complex<float> Y[8][8],B[8][8],D[8][8],X[8][8],c[8];
float base=100;
float vmag[8],ang[8],pload[8],qload[8],pgen[8],qgen[8];
float Pmismatch[8]={0},Qmismatch[8]={0},newvoltage[8]={0},newangle[8]={0};
int busno[8],bustype[8];
float k[8][8]={0},bb[8][8]={0},dd[8][8]={0},cc[8][8]={0};
float maxerror=1;
int counter1,slack,pv;
int m1=1,m2=1,p=0;
float P=0,Psigma=0;
float Qq=0, Qsigma=0;
int l,n,e,q;
...
while (maxerror>=conv)
{
	//calculate power mismatch
		       for(int i=1;i<=bus_no;i++)
			   {
				...
				if(bustype[i]==1)
				{	l=busno[i];
					Pmismatch[l]=pgen[i]/100-pload[i]/100-Psigma;
				}
			   ...
			   //cout<<Pmismatch[2]<<" "<<Pmismatch[3]<<" "<<Pmismatch[4]<<" "<<Pmismatch[5]<<" "<<endl;
			  // cout<<endl;
		   ...
		   for (int i=1;i<=bus_no;i++)
		   {
		     ...
		     //calculate Q mismatch
		   		for(int i=1;i<=bus_no;i++)
			   {
				...
				if(bustype[i]==1)
					{	l=busno[i];
					Qmismatch[l]=qgen[i]/100-qload[i]/100-Qsigma;
					//cout<<"Qmismatch:"<<endl;
					//cout<<Qmismatch[l]<<"  ";
					}
		   ...
		   if (bustype[lll]==0 || bustype[lll]==2)
			...
			//cout<<"new voltage at "<<lll<<" is "<<newvoltage[lll]<<endl;
			change=0;
			ll=1;
			lll++;
			 }
			...
			for (int i=1;i<=bus_no;i++)
		   {
			   if (bustype[i]!=0 || bustype[i]!=2)
				...
				//cout<<"voltage at bus "<<i<<" is "<<vmag[i]<<endl;
		   }
		   //cout<<endl;
				...
				for(int j=0;j<counter;j++)
					   {
						...
						P=P+(vmag[i]*vmag[p]*abs(Y[i][p])*cos(arg(Y[i][p])-ang[i]+ang[p]))*100;
						Qq=Qq+(vmag[i]*vmag[p]*abs(Y[i][p])*sin(arg(Y[i][p])-ang[i]+ang[p]))*100;

						   }
			pgen[i]=(pow(vmag[i],2)*abs(Y[i][i])*cos(arg(Y[i][i])))*100+P;
			qgen[i]=-(pow(vmag[i],2)*abs(Y[i][i])*sin(arg(Y[i][i])))*100-Qq;
					   }
				...
				for(int j=0;j<counter;j++)
					   {
						...
						P=P+(vmag[i]*vmag[p]*abs(Y[i][p])*sin(arg(Y[i][p])-ang[i]+ang[p]))*100;
						   }
			qgen[i]=-(pow(vmag[i],2)*abs(Y[i][i])*sin(arg(Y[i][i])))*100-P;
					   }
...
for(int i=1;i<=bus_no;i++)
{
	for(int j=1;j<=bus_no;j++)
	{
		...
		k[i][j]=real(X[i][j])*100;
		bb[i][j]=imag(X[i][j])*100;
